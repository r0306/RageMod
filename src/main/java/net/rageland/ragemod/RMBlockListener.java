...
public class RMBlockListener extends BlockListener 
{
    	...
    	if( block.getType() == Material.BED_BLOCK )
    	{
			...
			if( plugin.zones.isInZoneA(block.getLocation()) )
			{
				for( Lot lot : playerData.lots )
				{
					if( lot.canSetHome() && lot.isInside(block.getLocation()) )
					{
		    			...
		    			plugin.text.send(player, "You no longer have a home point.");
			...
			else if( plugin.zones.isInZoneB(block.getLocation()) )
			{
	    		...
	    		if( playerTown != null )
	    		{
	    			...
	    			{
		    			plugin.text.send(player, "You no longer have a spawn point.");
	    				playerData.clearSpawn();
		    			playerData.update();
	    			}
	    			else
	    			{
	    				plugin.text.sendNo(player, "You can only move your own bed.");
	    				event.setCancelled(true);
	    			}
	    		}
			}
    			...
    			if( playerData.isMember )
    			{
    				for( Lot lot : playerData.lots )
    				{
    					if( lot.canSetHome() && lot.isInside(block.getLocation()) )
    					{
    						...
    						{
    							plugin.text.sendNo(player, "You already have a bed inside your lot (use /home clear to fix errors).");
    							event.setCancelled(true);
    							return;
    						}
    		    			...
    		    			plugin.text.send(player, "Your home location has now been set.");
    		    			// Update both memory and database
    		    			playerData.update();
    					}
    				}
    	...
    	else if( plugin.zones.isInZoneB(location) )
    	{
			...
			if( block.getType() == Material.BED_BLOCK )
			{
	    		...
	    		if( playerTown != null && playerTown.townName.equals(playerData.townName) )
	    		{
					...
					{
						plugin.text.sendNo(player, "You already have a bed inside your town (use /spawn clear to fix errors).");
						event.setCancelled(true);
						return;
					}
	    			...
	    			for( String resident : spawns.keySet() )
	    			{
	    				...
	    				{
	    					plugin.text.parseNo(player, "This bed is too close to " + plugin.players.get(resident).getCodedName() + "'s bed - spawn not set.");
	    					event.setCancelled(true);
	    					return;
	    				}
	    			}
	    			...
	    			plugin.text.send(player, "Your spawn location has now been set.");
	    			// Update both memory and database
	    			plugin.database.playerQueries.updatePlayer(playerData);
	    		}
			}
    ...
    private boolean canEditBlock(BlockEvent event, Player player)
    {
    	...
    	if( plugin.zones.isInZoneA(location) && plugin.zones.isInCapitol(location) )
    	{
    		...
    		{
    			plugin.text.sendNo(player, "You don't have permission to mine under the city.");
    			return false;
    		}
    		...
    		else if( !playerData.isInsideLot(location) && !RageMod.permissionHandler.has(player, "ragemod.build.anylot") )
    		{
    			...
    			if( lot == null )
    			{
    				...
    				plugin.text.sendNo(player, "You don't have permission to edit city infrastructure.");
    			}
    			else
    			{
    				if( lot.owner.equals("") )
    					plugin.text.sendNo(player, "You cannot edit unclaimed lots.");
    				else
    				{
    					...
    					if( !ownerData.lotPermissions.contains(playerData.name) )
    						plugin.text.parseNo(player, "This lot is owned by " + plugin.players.get(lot.owner).getCodedName() + ".");
    					else
    						return true;
    				}
    			}
    	...
    	else if( plugin.zones.isInZoneB(location) )
    	{
    		...
    		if( playerTown != null )
    		{
    			...
    			{		
    				plugin.text.sendNo(player, "You can only build inside of your own town.");
    				return false;
    			}
    			else if( playerTown.isInsideSanctumFloor(location) )
    			{
    				plugin.text.sendNo(player, "You cannot modify the inner sanctum floor.");
    				return false;
    			}
    			else if( playerTown.isInsideSanctum(location) )
    			{
    				...
    				{
    					plugin.text.sendNo(player, "Only torches and gold blocks can be placed inside the inner sanctum.");
        				return false;
    				}
    	...
    	{
    		plugin.text.sendNo(player, "You cannot build inside the Travel Zone.");
    		return false;
    	}
